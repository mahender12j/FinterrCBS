...
docker run -d --name provisioner-ms --env eureka.instance.hostname=172.16.238.2 --env ribbon.listOfServers=172.16.238.2:9090 \
  ...
  --env cassandra.contactPoints=206.189.45.92:9042 --env mariadb.host=68.183.191.246 --network externaltools_app_net \
  --ip 172.16.238.6 provisioner:latest
cd ..

echo "Generating RSA keys..."
while ! docker exec -it provisioner-ms grep -q "provisioner-logger" /usr/local/logs/provisioner.log; do
  sleep 1
done

docker exec -it provisioner-ms sed -n -e 's/.*provisioner-logger - system.publicKey.//p' /usr/local/logs/provisioner.log | \
