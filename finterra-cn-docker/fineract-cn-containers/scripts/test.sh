...
function assign-customer-ms {
    local tenant="$1"

    ADMIN_PASSWORD=$( curl -s -H "Content-Type: application/json" -H "User: wepemnefret" -H "Authorization: ${TOKEN}" \
        --data '{ "name": "'"$CUSTOMER_MS_NAME"'" }' \
        ${PROVISIONER_URL}/tenants/malaysia/customerservice | jq --raw-output '.adminPassword')
    echo "Assigned identity microservice for tenant $tenant"
}
...
function create-user {
    ...
    curl -s -H "Content-Type: application/json" -H "User: $user" -H "Authorization: ${TOKEN}" -H "X-Tenant-Identifier: malaysia" \
        --data '{
...
function create-customer {
    ...
    #local access_token="$3"
     local tenant="$1"
    #curl -s "Content-Type: application/json" -H "User: $user" -H "Authorization: ${ACCESS_TOKEN}" -H "X-Tenant-Identifier: malaysia" \
     #   --data '[{ "firstname": "'"$service"'" }]' \
...
#	echo ""
    echo "$tenant services: "
    curl -s -H "Content-Type: application/json" -H "User: wepemnefret" -H "Authorization: ${TOKEN}" -H "X-Tenant-Identifier: $tenant" ${CUSTOMER_URL}/customers | jq '.'    
...
#create-application $REPORT_MS_NAME $REPORT_MS_DESCRIPTION $REPORT_MS_VENDOR $REPORT_URL
 ...
 #   read -p "Enter a name for the tenant, ${TENANT}: " -r name
  ...
  #  assign-identity-ms "malaysia"
    #assign-customer-ms "malaysia"
  # login "malaysia" "wepemnefret" "oS/0IiAME/2unkN1momDrhAdNKOhGykYFH/mJN20"
  #  echo "login with antony complted" $ADMIN_PASSWORD
    #create-scheduler-role "malaysia"
 #   echo "scheduler role created"
 #  create-user "malaysia" "wepemnefret" "charan" "aW5pdDFAbCA=" "orgAdmin"
  #  echo "imhotep created" 
  #  login "malaysia" "imhotep" "cDRzc3cwcmQ="
  #  update-password "malaysia" "imhotep" "cDRzc3cwcmQ="
  # login "malaysia" "charan" "aW5pdDFAbCA="
 #   create-customer "malaysia"
 # get-users "malaysia" "imhotep"
 #  get-tenants
...
#    login "malaysia" "antony" $ADMIN_PASSWORD
 ...
 #  create-user "malaysia" "antony" "charan" "aW5pdDFAbDIz" "orgadmin"
  #  login "malaysia" "antony" "cDRzc3cwcmQ="
