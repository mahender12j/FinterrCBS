...
public class CustomerRestController {
    ...
    ResponseEntity<Customer> fetchUserByIdentifier(@PathVariable("identifier") final String identifier) {
        return ResponseEntity.ok(this.customerService.getCustomerEntity(identifier).map(CustomerMapper::map).orElseGet(Customer::new));
    }
    ...
    @Permittable(value = AcceptedTokenType.TENANT, groupId = PermittableGroupIds.CUSTOMER)
    @RequestMapping(
            value = "/customers/{identifier}/aml",
            method = RequestMethod.POST,
            produces = MediaType.APPLICATION_JSON_VALUE,
            consumes = MediaType.APPLICATION_JSON_VALUE
    )
    public
    @ResponseBody
    ResponseEntity<Void> postAmlDetail(@PathVariable("identifier") final String identifier,
                                       @RequestBody @Valid final AmlDetail amlDetail) {
        if (this.customerService.customerExists(identifier)) {
