...
public class PermittableGroupRestController {
    ...
    ResponseEntity<Void> create(@RequestBody @Valid final PermittableGroup instance) {
        ...
        this.commandGateway.process(new CreatePermittableGroupCommand(instance));
        return new ResponseEntity<>(HttpStatus.ACCEPTED);
    }


    @RequestMapping(
            value = PathConstants.IDENTIFIER_RESOURCE_STRING,
            method = RequestMethod.PUT,
            consumes = {MediaType.APPLICATION_JSON_VALUE},
            produces = {MediaType.APPLICATION_JSON_VALUE})
    @Permittable(value = AcceptedTokenType.SYSTEM)
    public @ResponseBody
    ResponseEntity<Void> update(@RequestBody @Valid final PermittableEndpoint endpoint,
                                @PathVariable("identifier") final String identifier) {
        if (identifier == null)
            throw ServiceException.badRequest("Instance may not be null.");

        if (service.findByIdentifier(identifier).isPresent())
            this.commandGateway.process(new UpdatePermittableGroupCommand(endpoint, identifier));
        else
            throw ServiceException.conflict("Instance already exists with identifier:" + identifier);
        return new ResponseEntity<>(HttpStatus.ACCEPTED);
    }
    ...
    @RequestMapping(value = PathConstants.IDENTIFIER_RESOURCE_STRING,
    ...
    @Permittable(value = AcceptedTokenType.TENANT, groupId = PermittableGroupIds.ROLE_MANAGEMENT)
    public @ResponseBody
    ResponseEntity<PermittableGroup> get(@PathVariable(PathConstants.IDENTIFIER_PATH_VARIABLE) final String identifier) {
