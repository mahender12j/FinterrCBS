...
public class JournalRestController {
    ...
    @Permittable(value = AcceptedTokenType.TENANT, groupId = PermittableGroupIds.THOTH_JOURNAL)
    @RequestMapping(
            value = "/statistics/{creditorsAccountNumber}",
            method = RequestMethod.GET,
            produces = {MediaType.APPLICATION_JSON_VALUE},
            consumes = {MediaType.ALL_VALUE}
    )
    @ResponseBody
    ResponseEntity<List<JournalEntry>> fetchJournalEntriesForStatistics(
            @PathVariable(value = "creditorsAccountNumber") final String creditorsAccountNumber) {
        return ResponseEntity.ok(this.journalEntryService.fetchJournalEntriesForStatistics(creditorsAccountNumber));
    }
